...
class TestSecretsConfiguration:
    ...
    def test_required_secrets_exist(self):
        ...
        for required_secret in self.REQUIRED_SECRETS:
            assert required_secret in configured_secrets, f"Secret {required_secret} should be configured"
...
class TestSecretsUsageInWorkflows:
    ...
    def test_ci_cd_workflow_references_required_secrets(self):
        ...
        for secret_name in ["COOLIFY_WEBHOOK", "COOLIFY_TOKEN"]:
            ...
            assert proper_syntax in workflow_content, f"Should reference {secret_name} using proper syntax"
            
            # Ensure no hardcoded values (basic check)
            assert f"{secret_name}=" not in workflow_content, f"Should not have hardcoded {secret_name}"
    ...
    def test_secrets_are_used_in_environment_variables(self):
        ...
        for secret_name in ["COOLIFY_WEBHOOK", "COOLIFY_TOKEN"]:
            ...
            assert env_pattern in workflow_content, f"Should have proper env mapping for {secret_name}"
    ...
    def test_workflow_has_secret_validation_step(self):
        ...
        assert "Validate deployment secrets" in workflow_content, "Should have secret validation step"
        
        # Should check for empty secrets
        for secret_name in ["COOLIFY_WEBHOOK", "COOLIFY_TOKEN"]:
            validation_check = f'[ -z "${secret_name}" ]'
            assert validation_check in workflow_content, f"Should validate {secret_name} is not empty"
    ...
    def test_workflow_has_error_handling_for_missing_secrets(self):
        ...
        assert "::error::" in workflow_content, "Should have error reporting for missing secrets"
        assert "exit 1" in workflow_content, "Should exit with error code if secrets missing"
            ...
            for i, line in enumerate(lines):
                if 'name:' in line.lower():
                    for indicator in secret_indicators:
                        if indicator in line.lower() and any(char.isdigit() for char in line):
                            # If step name contains both secret indicator AND numbers, might be suspicious
                            print(f"⚠️  Review step name in {workflow_file.name}:{i+1}: {line.strip()}")
